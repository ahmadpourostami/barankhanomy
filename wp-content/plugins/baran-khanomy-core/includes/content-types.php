...
function bk_save_content_meta( $post_id ) {
    ...
    $fields = array( '_bk_testimonial_role' => 'sanitize_text_field', '_bk_testimonial_avatar' => 'esc_url_raw', '_bk_testimonial_rating' => 'absint', '_bk_student_work_student' => 'sanitize_text_field', '_bk_student_work_url' => 'esc_url_raw' );
...
function bk_category_add_image_field() { ?>
    <div class="form-field">
        <label for="bk-category-image">تصویر دسته‌بندی</label>
        <input type="hidden" name="bk_category_image_id" id="bk-category-image" value="">
        <div id="bk-category-image-preview"></div>
        <button type="button" class="button bk-category-select-image">انتخاب تصویر</button>
        <p>تصویر این دسته در کارت دسته‌بندی صفحه اصلی نمایش داده می‌شود.</p>
    </div>
<?php }

add_action( 'course-category_edit_form_fields', 'bk_category_edit_image_field' );
function bk_category_edit_image_field( $term ) {
    $image_id = (int) get_term_meta( $term->term_id, '_bk_category_image_id', true );
    $image_url = $image_id ? wp_get_attachment_image_url( $image_id, 'medium' ) : '';
    ?>
    <tr class="form-field">
        <th scope="row"><label for="bk-category-image">تصویر دسته‌بندی</label></th>
        <td>
            <input type="hidden" name="bk_category_image_id" id="bk-category-image" value="<?php echo esc_attr( $image_id ); ?>">
            <div id="bk-category-image-preview" style="margin-bottom:10px;"><?php if ( $image_url ) : ?><img src="<?php echo esc_url( $image_url ); ?>" style="width:120px;height:80px;object-fit:cover;border-radius:8px;"><?php endif; ?></div>
            <button type="button" class="button bk-category-select-image">انتخاب تصویر</button>
            <button type="button" class="button bk-category-remove-image" <?php echo $image_id ? '' : 'style="display:none;"'; ?>>حذف تصویر</button>
            <p class="description">این تصویر در بخش دسته‌بندی دوره‌های صفحه اصلی استفاده می‌شود.</p>
        </td>
    </tr>
    <?php
}

add_action( 'created_course-category', 'bk_save_category_image' );
add_action( 'edited_course-category', 'bk_save_category_image' );
function bk_save_category_image( $term_id ) {
    if ( ! current_user_can( 'manage_categories' ) ) return;
    if ( ! isset( $_POST['bk_category_image_id'] ) ) return;
    $image_id = absint( $_POST['bk_category_image_id'] );
    if ( $image_id ) update_term_meta( $term_id, '_bk_category_image_id', $image_id ); else delete_term_meta( $term_id, '_bk_category_image_id' );
}

add_action( 'admin_enqueue_scripts', 'bk_category_media_scripts' );
function bk_category_media_scripts( $hook ) {
    if ( ! in_array( $hook, array( 'edit-tags.php', 'term.php' ), true ) ) return;
    $taxonomy = isset( $_GET['taxonomy'] ) ? sanitize_key( $_GET['taxonomy'] ) : '';
    if ( 'course-category' !== $taxonomy ) return;
    wp_enqueue_media();
    wp_add_inline_script( 'jquery', "jQuery(function($){let frame;$('.bk-category-select-image').on('click',function(e){e.preventDefault();const b=$(this);frame=wp.media({title:'انتخاب تصویر دسته‌بندی',button:{text:'استفاده از تصویر'},multiple:false,library:{type:'image'}});frame.on('select',function(){const a=frame.state().get('selection').first().toJSON();$('#bk-category-image').val(a.id);$('#bk-category-image-preview').html('<img src=\"'+a.url+'\" style=\"width:120px;height:80px;object-fit:cover;border-radius:8px;\">');$('.bk-category-remove-image').show();});frame.open();});$('.bk-category-remove-image').on('click',function(){ $('#bk-category-image').val('');$('#bk-category-image-preview').empty();$(this).hide();});});" );
}
...
function bk_seed_tutor_course_categories() {
    ...
    foreach ( $terms as $term ) if ( ! term_exists( $term, 'course-category' ) ) wp_insert_term( $term, 'course-category' );
}
...
function bk_seed_demo_content() {
    ...
    if ( empty( wp_count_posts( 'bk_testimonial' )->publish ) ) {
        ...
        foreach ( $reviews as $review ) {
            ...
            if ( $post_id && ! is_wp_error( $post_id ) ) { update_post_meta( $post_id, '_bk_testimonial_role', $review[2] ); update_post_meta( $post_id, '_bk_testimonial_rating', 5 ); }
        }
    }
    if ( empty( wp_count_posts( 'bk_student_work' )->publish ) ) {
        foreach ( array( 'نمونه‌کار کیف بنفش', 'نمونه‌کار کیف پارچه‌ای', 'نمونه‌کار کیف زرد', 'نمونه‌کار کیف کرم', 'نمونه‌کار کیف کلاسیک', 'نمونه‌کار کیف دوشی' ) as $work ) wp_insert_post( array( 'post_type' => 'bk_student_work', 'post_status' => 'publish', 'post_title' => $work ) );
    }
}
